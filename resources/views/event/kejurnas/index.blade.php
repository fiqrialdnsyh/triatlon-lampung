...
@section('content')
    <section class="bg-navy pt-24 pb-16 px-8 md:px-16 text-center relative overflow-hidden">
        <div class="relative z-10">
            ...
            <p class="text-white/70 font-semibold max-w-2xl mx-auto text-sm">Portal pendaftaran khusus atlet delegasi daerah melalui akun kontingen resmi Federasi Triathlon Indonesia.</p>
        </div>
    </section>
                ...
                <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                        ...
                        <form action="{{ route('event.kejurnas.buat_kontingen') }}" method="POST" class="space-y-3">
                            ...
                            <button type="submit" class="w-full bg-navy text-yellow py-2 text-xs font-black uppercase tracking-wider rounded transition-colors">Daftarkan Akun Kontingen</button>
                        </form>
                    </div>
                ...
                @forelse($events as $event)
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-md overflow-hidden flex flex-col hover:-translate-y-1 transition-transform duration-300">
                        ...
                        <div class="p-6 flex-1 flex flex-col justify-between">
                            ...
                            <a href="{{ route('event.kejurnas.show', $event->slug) }}" class="block w-full text-center bg-navy text-yellow py-3 rounded-xl font-black text-xs uppercase tracking-wider hover:bg-yellow hover:text-navy transition-colors shadow-sm">
                                @if(auth()->check() && auth()->user()->email == 'admin@triatlon.test')
                                    Monitoring & Validasi Kontingen
                                @else
