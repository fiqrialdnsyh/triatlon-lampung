...
<head>
    ...
    <title>Laporan Check-In Kejurnas - {{ $event->judul }}</title>
...
<body class="bg-white text-black p-8" onload="window.print()">
    ...
    <div class="no-print mb-6">
        <button onclick="window.print()" class="bg-navy text-white px-5 py-2.5 rounded font-black text-xs uppercase tracking-wider shadow-md">Cetak / Simpan PDF</button>
        <button onclick="window.close()" class="bg-gray-200 text-black px-5 py-2.5 rounded font-black text-xs uppercase tracking-wider ml-2 shadow-sm">Tutup Tab</button>
    </div>

    <div class="border-b-4 border-black pb-4 mb-6 flex justify-between items-end">
        <div>
            <h1 class="text-2xl font-black uppercase tracking-wider">Laporan Kehadiran Atlet (Kejurnas)</h1>
    ...
    <table class="w-full text-left text-sm border-collapse border border-gray-800">
        <thead>
            <tr class="bg-gray-200 text-black uppercase font-black text-xs">
                ...
                <th class="p-3 border border-gray-800">Delegasi / Kontingen</th>
        ...
        <tbody>
            ...
            @forelse($checkins as $checkin)
                <tr>
                    ...
                    <td class="p-3 border border-gray-800 font-bold uppercase">{{ $checkin->nama_lengkap }}</td>
                    <td class="p-3 border border-gray-800 font-bold uppercase">
                        {{ $checkin->user->name ?? 'Kontingen Tidak Diketahui' }}<br>
                        <span class="text-[10px] text-gray-500">{{ $checkin->asal_daerah }}</span>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="6" class="p-6 text-center font-bold uppercase tracking-widest text-gray-500 border border-gray-800">Belum ada data kehadiran atlet dari kontingen manapun.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
