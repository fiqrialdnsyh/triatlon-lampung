...
@section('title', 'Buat Event Kejurnas - FTI LAMPUNG')
...
@section('content')
    <section class="bg-navy py-12 px-4 md:px-16 min-h-screen">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-cream p-8 md:p-10 rounded-2xl shadow-xl border border-gray-200">
                <h2 class="font-oswald text-3xl font-bold uppercase text-navy tracking-wide mb-2">Buat Event Kejurnas Baru</h2>
                <p class="text-sm text-navy/70 mb-8 pb-6 border-b border-navy/10">Lengkapi data teknis pertandingan, nomor perlombaan, serta regulasi khusus untuk delegasi resmi kontingen.</p>
                ...
                <form action="{{ route('event.kejurnas.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="kategori_lomba" class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-sm font-semibold text-navy focus:outline-none focus:border-navy shadow-sm" placeholder="Contoh: Triathlon Sprint Elite" required>
                        </div>
                    ...
                    <div class="bg-white p-6 rounded-xl border border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-3">
                            <h4 class="text-xs font-black text-navy uppercase tracking-widest border-b border-gray-100 pb-2 mb-2">Rekening Registrasi Kontingen</h4>
                        </div>
                        <div>
                    ...
                    <div class="bg-white p-6 rounded-xl border border-gray-200">
                        <h4 class="text-xs font-black text-navy uppercase tracking-widest border-b border-gray-100 pb-2 mb-4">Penggolongan Biaya Registrasi Kontingen</h4>
                        <div id="container-golongan" class="space-y-3">
                            <div class="flex gap-3 items-center bg-gray-50 p-3 rounded-lg border border-gray-200">
                                <input type="text" name="nama_golongan[]" value="Biaya Pendaftaran Delegasi" class="w-1/2 bg-white border border-gray-300 rounded-md px-3 py-2 text-xs font-bold text-navy" required>
                                <input type="number" name="biaya_golongan[]" value="250000" class="w-1/2 bg-white border border-gray-300 rounded-md px-3 py-2 text-xs font-bold text-navy" required>
                        ...
                        <button type="button" onclick="tambahBarisGolongan()" class="mt-3 text-[10px] font-black bg-gray-100 border border-gray-300 text-navy px-3 py-1.5 rounded uppercase tracking-wider hover:bg-gray-200 transition-colors">+ Tambah Golongan Biaya</button>
                    </div>
    ...
    <script>
        function tambahBarisGolongan() {
            ...
            row.innerHTML = `
                <input type="text" name="nama_golongan[]" placeholder="Nama Golongan" class="w-1/2 bg-white border border-gray-300 rounded-md px-3 py-2 text-xs font-bold text-navy" required>
