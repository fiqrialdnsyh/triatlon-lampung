...
@section('content')
        ...
        <div class="max-w-7xl mx-auto">
            ...
            @if(auth()->check() && auth()->user()->email === 'admin@triatlon.test')
                <div class="space-y-6 mb-10">
                    ...
                    <!-- PANEL QR CHECK-IN ATLET & DAFTAR HADIR (DIKELOMPOKKAN PER KONTINGEN) -->
                    <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm mb-6">
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                            <div>
                                ...
                                <p class="text-[11px] text-navy/50 font-bold mt-0.5">Gunakan kamera untuk memindai tiket peserta. Data kehadiran akan dikelompokkan per kontingen.</p>
                            </div>
                            <div class="flex gap-2 w-full md:w-auto">
                                <a href="{{ route('event.kejurnas.checkin.print', $event->id) }}" target="_blank" class="flex-1 md:flex-none text-center bg-gray-50 text-navy hover:bg-gray-100 border border-gray-300 px-4 py-3 font-black text-xs uppercase tracking-wider rounded-xl shadow-sm transition-colors">
                                    Cetak PDF
                                </a>
                        ...
                        <!-- TABEL PESERTA YANG SUDAH CHECK-IN (PER KONTINGEN) -->
                        <div class="bg-gray-50/50 rounded-xl border border-gray-200 overflow-hidden">
                            ...
                            @forelse($groupedCheckins as $userId => $checkins)
                                @php $namaKontingen = $checkins->first()->user->name ?? 'Kontingen Tidak Diketahui'; @endphp

                                <div class="bg-navy p-3 flex justify-between items-center border-b border-navy/10">
                                    <h4 class="font-black text-white uppercase text-xs tracking-wider">DELEGASI HADIR: {{ $namaKontingen }}</h4>
                                    <span class="bg-yellow text-navy px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest">{{ $checkins->count() }} Atlet</span>
                                </div>
                                            ...
                                            @foreach($checkins as $checkin)
                                                <tr class="hover:bg-white transition-colors">
                                                    ...
                                                    <td class="p-3 text-center">
                                                        <p class="font-bold text-navy text-[10px] uppercase">{{ $checkin->jenis_kelamin }}</p>
                                                        <p class="text-[9px] text-red-600 font-black tracking-widest">{{ $checkin->golongan_darah }}</p>
                                                    </td>
                            ...
                            @empty
                                <div class="p-6 text-center text-navy/40 font-bold uppercase text-[10px] tracking-widest">Belum ada peserta yang melakukan Check-In dari kontingen manapun.</div>
                            @endforelse
                        </div>
                    </div>

                    <!-- TABEL MONITORING & VERIFIKASI PESERTA - GROUPING KONTINGEN -->
                    <div class="space-y-6">
                        <h3 class="font-black text-navy uppercase text-sm border-b border-gray-200 pb-2">Verifikasi Berkas Registrasi per Kontingen</h3>

                        @forelse($groupedRegistrations as $userId => $registrations)
                            @php $kontingenName = $registrations->first()->user->name ?? 'Kontingen Dihapus / Tidak Diketahui'; @endphp
                            ...
                            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                                ...
                                <div class="bg-navy p-4 flex justify-between items-center border-b border-gray-200">
                                    ...
                                    <span class="bg-yellow text-navy px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm">
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full text-left text-xs border-collapse min-w-[900px]">
                                        <thead>
                                            <tr class="bg-gray-50 text-navy uppercase font-bold text-[10px] tracking-wider border-b border-gray-200">
                                                <th class="p-4 rounded-l-md w-[45%]">Data Lengkap Profil Atlet</th>
                                                <th class="p-4 w-[20%]">Kategori & Tarif</th>
                                        ...
                                        <tbody class="divide-y-2 divide-gray-100 font-semibold text-navy">
                                            @foreach($registrations as $reg)
                                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-blue-50/40 transition-colors">

                                                    <td class="p-4">
                                                        <div class="mb-2 border-b border-gray-200/50 pb-2">
                                                            <div class="flex items-center gap-2 mb-1">
                                                                <p class="font-black uppercase text-sm text-navy tracking-wide">{{ $reg->nama_lengkap }}</p>
                                                                <span class="bg-navy text-yellow px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest">
                                                                    BIB: {{ $reg->bib_name }}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="grid grid-cols-2 gap-x-4 gap-y-3">
                                                            <div>
                                                                ...
                                                                <p class="text-[11px] font-semibold text-navy">{{ $reg->nomor_ktp }}</p>
                                                            </div>
                                                            <div>
                                                                <p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">TTL & Usia</p>
                                                                <p class="text-[11px] font-semibold text-navy">{{ \Carbon\Carbon::parse($reg->tanggal_lahir)->format('d/m/Y') }} ({{ $reg->usia }} Thn)</p>
                                                            </div>
                                                            <!-- REVISI: Menampilkan Gender dan Golongan Darah Secara Spesifik -->
                                                            <div>
                                                                <p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Jenis Kelamin</p>
                                                                <p class="text-[11px] font-black text-navy uppercase">{{ $reg->jenis_kelamin }}</p>
                                                            </div>
                                                            <div>
                                                                <p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Gol. Darah</p>
                                                                <p class="text-[11px] font-black text-red-600 uppercase">{{ $reg->golongan_darah }}</p>
                                                            </div>
                                                            <div class="col-span-2">
                                                                <p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Daerah Domisili</p>
                                                                <p class="text-[11px] font-semibold text-navy uppercase">{{ $reg->asal_daerah }}</p>
                                                            </div>
                                                        </div>
                                                    </td>

                                                    <td class="p-4 align-top pt-5 border-l border-gray-100">
                                                        <p class="font-black text-xs text-navy uppercase leading-tight mb-1">{{ $reg->kategori_lomba }}</p>
                                                        <span class="inline-block bg-white text-gray-600 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider mb-2 border border-gray-200">
                                                            {{ $reg->golongan_biaya }}
                                                        </span>
                                                        <p class="text-xs font-black text-green-600">Rp {{ number_format($reg->nominal_bayar, 0, ',', '.') }}</p>
                                                    </td>

                                                    <td class="p-4 text-center align-top pt-5 border-l border-gray-100">
                                                        <button type="button" onclick="openProofModal('{{ asset($reg->bukti_transfer) }}')" class="inline-flex items-center justify-center gap-1 bg-blue-50 hover:bg-blue-600 text-blue-600 hover:text-white border border-blue-200 hover:border-blue-600 px-3 py-1.5 rounded text-[9px] font-black uppercase tracking-wider transition-all duration-300">
                                                            Cek Bukti
                                                        </button>
                                                    </td>
                                                    ...
                                                    <td class="p-4 align-top pt-5 border-l border-gray-100">
                                                        @if($reg->status_pembayaran == 'Menunggu')
                                                            <form action="{{ route('event.kejurnas.verifikasi', $reg->id) }}" method="POST" class="flex flex-col items-end gap-2">
                                                                @csrf
                                                                <input type="text" name="pesan_penolakan" placeholder="Alasan jika ditolak..." class="bg-white border border-gray-300 rounded px-2 py-1.5 text-[10px] w-full font-semibold focus:outline-none focus:border-navy">
                                                                <div class="flex gap-1.5 w-full">
                                                                    <button type="submit" name="status_pembayaran" value="Valid" class="w-1/2 bg-green-500 hover:bg-green-600 text-white py-1.5 rounded text-[9px] uppercase font-black transition-colors cursor-pointer">Terima</button>
                                                                    <button type="submit" name="status_pembayaran" value="Ditolak" class="w-1/2 bg-red-500 hover:bg-red-600 text-white py-1.5 rounded text-[9px] uppercase font-black transition-colors cursor-pointer">Tolak</button>
                                                                </div>
                                                            </form>
                                                        @else
                                                            <div class="flex flex-col items-end justify-start gap-1">
                                                                <span class="px-4 py-1.5 rounded text-[9px] font-black uppercase tracking-widest block w-full text-center border {{ $reg->status_pembayaran == 'Valid' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200' }}">
                                                                    STATUS: {{ $reg->status_pembayaran }}
                                                                </span>
                                                                <form action="{{ route('event.kejurnas.verifikasi', $reg->id) }}" method="POST" class="w-full text-right mt-1">
                                                                    @csrf
                                                                    <button type="submit" name="status_pembayaran" value="Menunggu" class="text-[9px] text-navy/40 font-bold hover:text-navy hover:underline cursor-pointer w-full text-right">Batalkan (Reset)</button>
                                                                </form>
                                                            </div>
                                                        @endif
                                                    </td>
                        ...
                        @empty
                            <div class="bg-white border-2 border-dashed border-gray-300 p-12 rounded-2xl text-center">
                                <p class="font-black text-navy/40 uppercase text-sm tracking-wider">Belum ada satupun kontingen yang mendaftarkan atlet.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
            ...
            <!-- ========================================================= -->
            <!-- STRUKTUR TAMPILAN LEVEL KONTINGEN (PENDAFTARAN BANYAK)    -->
            <!-- ========================================================= -->
            @else
                <div class="flex flex-col lg:flex-row gap-8">
                    ...
                    <!-- BLOK FORM REGISTRASI KONTINGEN -->
                    <div class="w-full lg:w-8/12">
                        @auth
                            @if(auth()->user()->role === 'kontingen')
                                ...
                                <!-- TABEL RANGKUMAN ATLET YANG SUDAH DIDAFTARKAN OLEH KONTINGEN INI -->
                                <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
    ...
    <!-- POPUP MODAL SCANNER QR KAMERA TEROPTIMASI -->
    @if(auth()->check() && auth()->user()->email === 'admin@triatlon.test')
    ...
    <script>
        function openProofModal(imageUrl) {
            ...
            document.body.style.overflow = 'hidden'; // Mencegah halaman utama ikut di-scroll
        }
