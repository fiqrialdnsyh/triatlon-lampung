...
<body class="bg-navy font-sans text-navy antialiased min-h-screen flex items-center justify-center relative overflow-hidden">
    ...
    <a href="{{ url('/') }}" class="absolute top-8 left-8 z-20 inline-flex items-center text-white/70 hover:text-yellow text-xs font-bold uppercase tracking-wider transition-colors">
    ...
    <div class="relative z-10 w-full max-w-md px-6">
        <div class="bg-cream rounded-[2rem] p-8 md:p-10 shadow-2xl">
            ...
            @error('email')
                <div class="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative text-xs font-bold" role="alert">
            ...
            <form action="{{ url('/login') }}" method="POST" class="space-y-5">
                @csrf <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3.5 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow focus:border-yellow transition-all shadow-sm" placeholder="nama@email.com" required>
                </div>

                <div>
                    <div class="flex justify-between items-center mb-2">
                        <label class="block text-xs font-bold text-navy uppercase tracking-wide">Kata Sandi</label>
                        <a href="#" class="text-[11px] font-bold text-navy/60 hover:text-navy transition-colors">Lupa Sandi?</a>
                    </div>
                    <input type="password" name="password" class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3.5 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow focus:border-yellow transition-all shadow-sm" placeholder="••••••••" required>
                </div>

                <div class="flex items-center">
                    <label class="flex items-center space-x-2 cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-4 h-4 text-navy focus:ring-yellow border-gray-300 rounded cursor-pointer">
                        <span class="text-xs font-bold text-navy/70 group-hover:text-navy transition-colors">Ingat Saya</span>
                    </label>
                </div>
            ...
            <div class="mt-8 text-center">
                <p class="text-xs font-semibold text-navy/70">
                    Belum punya akun?
                    <a href="{{ url('/register') }}" class="font-black text-navy uppercase hover:text-yellow transition-colors ml-1 border-b border-navy hover:border-yellow">Daftar Di Sini</a>
                </p>
            </div>
