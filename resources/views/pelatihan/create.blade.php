...
@section('content')
            ...
            <div class="bg-cream p-8 md:p-10 rounded-[2rem] shadow-2xl">
                ...
                @if($errors->any())
                    <div class="mb-8 bg-red-50 border-l-4 border-red-500 p-4 rounded-xl shadow-sm">
                        <p class="text-xs font-black text-red-700 uppercase mb-2">Gagal Menyimpan Pelatihan, Periksa Isian Berikut:</p>
                ...
                <form action="{{ route('pelatihan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="judul" value="{{ old('judul') }}" class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow focus:border-yellow transition-all shadow-sm" placeholder="Contoh: Kursus Pelatih Tingkat Dasar (Level 1)" required>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="lokasi" value="{{ old('lokasi') }}" class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow focus:border-yellow transition-all shadow-sm" placeholder="Contoh: GSG Kampus ITERA" required>
                            </div>
                            <div>
                                ...
                                <input type="number" name="kuota_maksimal" value="{{ old('kuota_maksimal') }}" class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow focus:border-yellow transition-all shadow-sm" placeholder="Contoh: 50" required>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="date" id="tanggal_pelaksanaan" name="tanggal_pelaksanaan" value="{{ old('tanggal_pelaksanaan') }}" min="{{ \Carbon\Carbon::now('Asia/Jakarta')->format('Y-m-d') }}" class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-sm font-semibold text-navy focus:outline-none" required>
                            </div>
                            <div>
                                ...
                                <input type="datetime-local" id="batas_pendaftaran" name="batas_pendaftaran" value="{{ old('batas_pendaftaran') }}" min="{{ \Carbon\Carbon::now('Asia/Jakarta')->format('Y-m-d\TH:i') }}" class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-sm font-semibold text-navy focus:outline-none" required>
                            </div>
                            <div>
                    ...
                    <div class="space-y-6 pt-6 border-t border-navy/10">
                        ...
                        <div class="bg-white/50 p-6 rounded-2xl border border-gray-300 space-y-4">
                            ...
                            <div id="container-biaya" class="space-y-3">
                                <div class="flex items-center gap-3 baris-biaya">
                                    <div class="flex-1">
                                        <input type="text" name="nama_golongan[]" class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow transition-all shadow-sm" placeholder="Nama Golongan (Contoh: Umum)" required>
                                    </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="rekening" value="{{ old('rekening') }}" class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm font-semibold text-navy focus:outline-none focus:ring-2 focus:ring-yellow focus:border-yellow transition-all shadow-sm" placeholder="Contoh: Bank Mandiri 114-00-1234567-8 a.n FOPI Lampung" required>
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <input type="url" name="link_wa_grup" value="{{ old('link_wa_grup') }}" placeholder="https://chat.whatsapp.com/..." class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-sm font-semibold text-navy focus:outline-none" required>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set batasan minimum hari ini berdasarkan jam sistem komputer klien (mencegah klik tanggal mundur)
            ...
            inputTglAcara.addEventListener('change', function() {
                if(this.value) {
                    // Paksa batas pendaftaran maksimal 1 hari sebelum (H-1) pelaksanaan
                    ...
                    // Kunci max dan set min
                    inputBatas.max = hMinusSatuDate + 'T23:59';

                    // Jika tanggal batas yang dipilih sebelumnya kelewat, otomatis di-reset
            ...
            // Trigger manual jika form gagal dan me-load ulang value lama (old)
