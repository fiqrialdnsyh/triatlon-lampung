...
<body class="bg-navy p-4 md:p-12 min-h-screen flex flex-col items-center justify-center">
    ...
    <div class="w-full max-w-3xl flex justify-between items-center mb-6 no-print">
        ...
        <button onclick="window.print()"
            class="bg-yellow text-navy px-6 py-2.5 font-black text-xs uppercase rounded-xl hover:bg-white transition-colors shadow-md">
            Cetak / Simpan PDF
        </button>
    </div>

    <div
        class="print-container w-full max-w-3xl bg-cream rounded-[2rem] shadow-2xl overflow-hidden border border-gray-200 flex flex-col md:flex-row">

        <div
            class="flex-1 p-8 md:p-10 flex flex-col justify-between border-b-2 md:border-b-0 md:border-r-2 border-dashed border-navy/20 relative">
            ...
            <div>
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <span class="block text-[10px] font-black text-navy/40 uppercase tracking-widest">OFFICIAL ENTRY
                            PASS</span>
                        <h2 class="text-xl font-black text-navy uppercase tracking-wide">TRIATLON LAMPUNG</h2>
                    </div>
                    <span
                        class="bg-navy text-yellow text-[9px] font-black uppercase px-2.5 py-1 rounded">VERIFIED</span>
                </div>

                <div class="mb-6">
                    <span class="text-[10px] font-bold text-navy/60 uppercase tracking-wider block mb-1">PROGRAM
                        KEGIATAN:</span>
                    <h1
                        class="font-oswald text-2xl md:text-3xl font-bold uppercase text-navy leading-tight tracking-wide">
                        {{ $pendaftaran->pelatihan->judul }}</h1>
                </div>

                <div class="grid grid-cols-2 gap-x-4 gap-y-4 border-t border-navy/10 pt-6">
                    <div>
                        <span class="text-[9px] font-bold text-navy/60 uppercase tracking-wider block">NAMA
                            PESERTA</span>
                        <span
                            class="text-sm font-black text-navy uppercase block">{{ $pendaftaran->nama_lengkap }}</span>
                    </div>
                    <div>
                        <span class="text-[9px] font-bold text-navy/60 uppercase tracking-wider block">KATEGORI
                            JALUR</span>
                        <span
                            class="text-sm font-black text-navy uppercase block">{{ $pendaftaran->golongan_biaya }}</span>
                    </div>
                    <div>
                        <span class="text-[9px] font-bold text-navy/60 uppercase tracking-wider block">ASAL
                            DAERAH</span>
                        <span
                            class="text-sm font-black text-navy uppercase block">{{ $pendaftaran->asal_daerah }}</span>
                    </div>
                    <div>
                        <span class="text-[9px] font-bold text-navy/60 uppercase tracking-wider block">UKURAN
                            BAJU</span>
                        <span class="text-sm font-black text-navy uppercase block">SIZE
                            {{ $pendaftaran->ukuran_baju }}</span>
                    </div>
                </div>
            </div>

            <div
                class="mt-8 pt-4 border-t border-navy/10 flex items-center justify-between text-[10px] font-semibold text-navy/60">
        ...
        <div
            class="w-full md:w-64 bg-navy p-8 md:p-10 text-white flex flex-col justify-between items-center text-center relative">

            <div>
                <span class="block text-[9px] font-black text-yellow uppercase tracking-widest mb-6">JADWAL
                    PELAKSANAAN</span>
