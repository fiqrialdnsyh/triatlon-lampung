...
@section('content')
    <section class="relative w-full h-[400px] md:h-[500px]">
        ...
        <a href="{{ url('/berita') }}" class="absolute top-8 left-8 z-20 inline-flex items-center text-white/90 hover:text-yellow text-xs font-bold uppercase tracking-wider transition-colors bg-navy/50 px-4 py-2 rounded-xl backdrop-blur-sm">
    ...
    <section class="bg-navy px-4 md:px-16 pb-24 relative z-10">
        <div class="max-w-4xl mx-auto bg-cream p-8 md:p-14 rounded-[2rem] shadow-2xl -mt-24">
            ...
            <div class="text-center mb-10 border-b-2 border-navy/10 pb-8">
                <div class="inline-block bg-yellow text-navy text-[10px] font-black px-3 py-1.5 rounded-sm uppercase tracking-wider mb-4 shadow-sm">
                    {{ $berita->kategori }}
                </div>
                <h1 class="font-oswald text-3xl md:text-5xl font-bold uppercase text-navy leading-tight mb-4">{{ $berita->judul }}</h1>
                <div class="flex items-center justify-center space-x-4 text-xs font-bold text-navy/60 uppercase tracking-widest">
            ...
            <article class="prose prose-navy max-w-none text-sm md:text-base font-semibold text-navy/80 leading-loose space-y-6">
                {!! $berita->konten !!}
            </article>
                ...
                <div class="flex flex-wrap gap-2">
                    @if($berita->tags)
                        @foreach(explode(',', $berita->tags) as $tag)
                            <span class="text-[10px] font-black text-white bg-navy px-3 py-1.5 rounded-md uppercase">#{{ trim($tag) }}</span>
                        @endforeach
                    @endif
                </div>
                <button onclick="bagikanArtikel()" class="inline-flex items-center text-xs font-black text-navy uppercase border-2 border-navy hover:bg-navy hover:text-white px-6 py-2 rounded-xl transition-colors group">
    ...
    <style>
        ...
        .prose strong { color: #0B1528; font-weight: 900; }
